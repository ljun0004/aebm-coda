...
def train_one_epoch(model, vae, multi_quantizer,
    ...
    for data_iter_step, (samples, labels) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            if args.vae_mode == "kl":
                ...
                cookbook = multi_quantizer.codebooks.weight
                # VAE encode
                if args.use_cached:
                    moments, gt_indices = samples
        ...
        with sync_context():
            with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
                total_loss, ddpmloss, celoss, reloss, logitsnorm, qnorm, pimax, scorenorm, tembnorm, scale = model(h, labels, gt_indices=gt_indices, cookbook=cookbook, warmup=warmup)
            
            # if data_iter_step < args.accum_iter:
...
def evaluate(model_without_ddp, vae, multi_quantizer, ema_params, args, epoch, batch_size=16, log_writer=None, cfg=1.0,
             use_ema=True):
...
def generate(model_without_ddp, vae, multi_quantizer, ema_params, args, epoch, batch_size=16, log_writer=None, cfg=1.0,
    ...
    if data_loader is not None:
        ...
        # print(f"generate: samples: {samples.shape, samples.min().item(), samples.max().item()}")
        # print(f"generate: h: {h.shape, h.min().item(), h.max().item()}")
    ...
    for i in range(num_steps):
        ...
        with torch.no_grad():
            ...
            with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
                ...
                # print(f"generate: sampled_images: {sampled_images.shape}")
        ...
        for b_id in range(sampled_images.size(0)):
            img_id = i * sampled_images.size(0) * world_size + local_rank * sampled_images.size(0) + b_id
            print(f"generate: img_id: {img_id}, args.gen_num_images: {args.gen_num_images}")
            ...
            print(f"generate: save_path: {save_path}")
    ...
    for data_iter_step, (samples, _, paths) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            ...
            if args.vae_mode == "kl":
                ...
                gt_indices_flip = token_indices_flip.view(h_flip.size(0), -1).long()
        ...
        for i, path in enumerate(paths):
            ...
            if args.vae_mode == "kl":
                ...
                np.savez(save_path, moments=moments[i].cpu().numpy(), gt_indices = gt_indices[i].cpu().numpy(), moments_flip=moments_flip[i].cpu().numpy(), gt_indices_flip = gt_indices_flip[i].cpu().numpy())
            elif args.vae_mode == "vq":
